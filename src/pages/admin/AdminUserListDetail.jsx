...
import { Avatar,Paper, Typography, Box, TextField, Button, ThemeProvider,Dialog, DialogTitle, List, ListItemButton, DialogContent, DialogActions } from "@mui/material";
...
const DeleteDialog = (props) => {
  ...
  return(
    <Dialog open={open} onClose={OnCloseHandler}>
      ...
      <DialogContent>
        한번 삭제하면 다시 되돌릴 수 없습니다. 그래도 삭제하시겠습니까?
      </DialogContent>
      <DialogActions>
        <Button color='error' onClick={OnClickUserDelete}>삭제</Button>
...
const RoleDialog = (props) => {
  ...
  const OnClickRole = async (role) => {
    try{
      const response = await axiosInstance.put(`/admin/users/${userDetail.id}`, null, {params: {updateRole : role}});
  ...
  return (
    <MainContainer>
      ...
      <Typography variant="body1" sx={{display: 'inline-block', margin: '0px 0px 0px 15px'}}>
        {userDetail.name}<br/>{GetUserRoleString(userDetail.role)}
      </Typography>
      <Box sx={{margin: '20px 0px 15px 0px', border: '1px solid grey' }}></Box>
      <TextField disabled fullWidth label='이메일' defaultValue={userDetail.email}sx={{margin: '5px 0 auto'}}></TextField>
      <TextField disabled fullWidth label='연락처' defaultValue={userDetail.phoneNumber} sx={{margin: '15px 0 auto'}}></TextField>
      <TextField disabled fullWidth label='주소' defaultValue={userDetail.address} sx={{margin: '15px 0 auto'}}></TextField>
      <TextField disabled fullWidth label='상세 주소' defaultValue={userDetail.addressDetail} sx={{margin: '15px 0 auto'}}></TextField>
      <ThemeProvider theme={theme}>
      <Button variant="contained" color='black' sx={{margin: '10px 0 auto'}} onClick={(e) => OpenRoleDialog(true)}>권한 변경</Button>
      <Button variant="contained" color='error' sx={{margin: '10px 0 auto', float: 'right'}} onClick={(e) => {OpenDeleteDialog(true)}}>회원 삭제</Button>
      </ThemeProvider>
